...
export const AqiForecastView: React.FC<AqiForecastViewProps> = ({
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="bg-[#0a0a0a] border border-white/10 p-5 flex flex-col md:flex-row md:items-baseline justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-1.5 py-0.5 bg-red-600/20 border border-red-500/40 text-red-400 text-[9px] font-black uppercase">
              24H – 72H RATE ENGINE
            </span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tighter text-white uppercase mt-1">
            POLLUTION RATE PREDICATION & FORECAST
          </h1>
        ...
        <div className="flex items-center gap-3 self-start md:self-auto">
          <div className="relative">
            <select
              ...
              className="bg-white text-black text-xs font-black uppercase tracking-wider px-3.5 py-2 rounded-none border border-white appearance-none pr-8 cursor-pointer focus:outline-none"
            >
              {stations.map((st) => (
          ...
          <button
            ...
            className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-black uppercase tracking-wider transition-all flex items-center gap-1.5 shadow-[0_0_15px_rgba(220,38,38,0.3)]"
          >
      ...
      {/* 2. 24H, 48H, 72H Rate Prediction Metric Tiles */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* +24 Hours Rate Card */}
        <div className="bg-[#0f0f0f] border border-white/10 p-5 space-y-3 relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50">
              +24 HOURS PREDICATION
            </span>
            <span className={`text-[9px] font-black uppercase px-2 py-0.5 ${rateDelta24 <= 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
          ...
          <div className="flex items-baseline justify-between">
            <div className="text-4xl font-black font-mono text-white tracking-tighter">
              {h24Aqi} <span className="text-xs font-normal opacity-50">AQI</span>
            </div>
          ...
          <div className="text-xs text-white/70 leading-relaxed font-mono pt-2 border-t border-white/5">
            <strong>PBL Mixing:</strong> Solar convection lifts inversion layer up to 950m during midday, temporarily diluting fine particulates.
          </div>
        </div>
        ...
        <div className="bg-[#0f0f0f] border border-white/10 p-5 space-y-3 relative overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-yellow-500/20 text-yellow-400">
              RE-ACCUMULATION
            </span>
          </div>
          ...
          <div className="flex items-baseline justify-between">
            <div className="text-4xl font-black font-mono text-white tracking-tighter">
              {h48Aqi} <span className="text-xs font-normal opacity-50">AQI</span>
            </div>
          ...
          <div className="text-xs text-white/70 leading-relaxed font-mono pt-2 border-t border-white/5">
            <strong>Stagnation Risk:</strong> Wind speeds decline below 4.0 km/h; secondary inorganic aerosols (sulfates/nitrates) build up across NCR.
          </div>
        </div>

        {/* +72 Hours Rate Card */}
        <div className="bg-[#0f0f0f] border-2 border-red-600/60 p-5 space-y-3 relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-red-400">
              +72 HOURS PREDICATION (PEAK)
            </span>
            <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-red-600 text-white animate-pulse">
              SEVERE SURGE
            </span>
          </div>

          <div className="flex items-baseline justify-between">
            <div className="text-4xl font-black font-mono text-red-500 tracking-tighter">
              {h72Aqi} <span className="text-xs font-normal text-white opacity-50">AQI</span>
            </div>
          ...
          <div className="text-xs text-white/70 leading-relaxed font-mono pt-2 border-t border-white/5">
            <strong>GRAP Stage IV Alert:</strong> High probability of exceeding 400 AQI benchmark. Immediate emergency contingency required.
          </div>
        </div>
      </div>

      {/* 3. Diurnal Hourly Rate Timeline (WRF-Chem Trajectory) */}
        ...
        <div className="space-y-2.5">
          {ratePoints.map((pt, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-black/60 border border-white/10 p-3.5 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:border-white/25 transition-all"
              >
                {/* Time & Stage */}
                <div className="w-48 shrink-0">
                  <div className="text-xs font-black text-white uppercase">{pt.timestamp}</div>
                  <span
                    className={`text-[9px] font-mono font-bold uppercase px-1.5 py-0.5 mt-0.5 inline-block ${
                ...
                {/* Description */}
                <div className="flex-1 text-xs text-white/80 font-sans">
                ...
                {/* AQI & Rate */}
                <div className="flex items-center gap-6 shrink-0 justify-between md:justify-end border-t md:border-t-0 pt-2 md:pt-0 border-white/5">
                  <div className="text-right">
                    <span className="text-[9px] font-mono opacity-50 block uppercase">PM2.5 / PM10</span>
                  ...
                  <div className="text-right w-20">
                    <span className="text-[9px] font-mono opacity-50 block uppercase">RATE DELTA</span>
                    <span
                  ...
                  <div className="text-right w-16">
                    ...
                    <span className="text-[9px] font-mono opacity-50 uppercase">AQI</span>
                  </div>
                </div>
              </div>
            );
      ...
      {/* 4. Multi-Station 72-Hour Comparison Table */}
      <div className="bg-[#0f0f0f] border border-white/10 p-5 space-y-4">
            ...
            <tbody className="divide-y divide-white/5 font-mono text-white/80">
              {stations.map((st) => (
                <tr key={st.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="py-2.5 px-3 font-sans text-white/50">{st.district}</td>
